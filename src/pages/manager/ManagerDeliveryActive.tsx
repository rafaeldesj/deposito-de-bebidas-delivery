...
export default function ManagerDeliveryActive() {
  ...
  useEffect(() => {
    ...
    // Subscribe to realtime updates on public:orders for delivering status
    const channel = supabase
      .channel('delivering_orders_changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'orders', filter: 'status=eq.delivering' }, loadActiveOrders)
      .subscribe();
  ...
  useEffect(() => {
    ...
    const depositoMarker = L.marker(STORE_COORDS, { icon: originIcon })
      .addTo(map)
      .bindPopup('<b>Depósito de Bebidas Delivery</b><br/>Ponto de Origem 🍻');
    layersRef.current.push(depositoMarker);

    const boundsPoints: L.LatLngExpression[] = [STORE_COORDS];
    ...
    orders.forEach((order) => {
      ...
      if (riderCoords) {
        ...
        if (order.clientCoords) {
          ...
          // Desenha polilinha entre Depósito -> Entregador -> Cliente
    ...
    <div className="dashboard-layout animate-fade-in" style={{ padding: '0.5rem 0' }}>
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <p style={{ margin: 0, fontSize: '0.88rem' }}>Os entregadores estão aguardando novos pedidos no depósito.</p>
        </div>
      ) : (
        <>
          {/* Aba do Mapa Geral */}
          {activeTab === 'map' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', padding: '0.5rem 0.25rem', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>🏠 Depósito de Bebidas</span>
