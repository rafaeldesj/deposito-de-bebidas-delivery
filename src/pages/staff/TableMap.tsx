...
import { mapUserFromDb } from '../../context/AuthContext';
...
export const TableMap = () => {
  ...
  const mapReservationFromDb = (row: any) => {
    return {
  ...
  const mapReservationToDb = (res: any) => {
    return {
    ...
    const loadConfig = async () => {
      try {
        const { data } = await supabase
          ...
          .maybeSingle();
        if (data && data.value) {
          setActiveTablesCount((data.value as any).activeTablesCount || 99);
        } else {
  ...
  useEffect(() => {
    const loadClients = async () => {
      const { data, error } = await supabase
        ...
        .eq('role', 'client');
      if (!error && data) {
        setAllClients(data.map(mapUserFromDb));
      }
    };
    loadClients();

    const channel = supabase
      .channel('users_clients')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'users', filter: 'role=eq.client' }, loadClients)
      .subscribe();
  ...
  useEffect(() => {
    const loadReservations = async () => {
      const { data, error } = await supabase
        ...
        .select('*');
      if (!error && data) {
        setReservations(data.map(mapReservationFromDb));
      }
    };
    loadReservations();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'orders', filter: 'order_type=eq.dine_in_table' }, loadActiveOrders)
      .subscribe();
  ...
  const handleUpdateActiveTables = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await supabase.from('settings').upsert({
        key: 'tables_config',
        value: { activeTablesCount: finalVal, updatedAt: new Date().toISOString() }
      });
    } catch (err) {
  ...
  const handleApproveCashierOrder = async (order: OrderDocument) => {
    ...
    try {
      ...
      const { error: txErr } = await supabase.from('transactions').insert({
        ...
        type: 'baixa_manual',
        approved_by: userData?.name || userData?.email || 'Caixa'
      });
    ...
    try {
      ...
      const { error: cancelErr } = await supabase
        ...
        .update({
          ...
          cancelled_by: userData?.name || userData?.email || 'Gerente',
        })
        .eq('id', orderId);
      ...
      if (order) {
        ...
        if (!otherActive && order.tableNumber) {
          const { data: usersAtTable, error: queryErr } = await supabase
            .from('users')
            .select('uid')
            .eq('table_number', order.tableNumber);
            
          if (!queryErr && usersAtTable) {
            const batchPromises = usersAtTable.map(userDoc => 
              supabase
                .from('users')
                .update({
                  table_number: null,
    ...
    try {
      ...
      const userPromises = linkedUsers.map(u => 
        supabase
          .from('users')
          .update({
            table_number: String(targetTableNum),
            updated_at: new Date().toISOString()
          })
          .eq('uid', u.uid)
      );
