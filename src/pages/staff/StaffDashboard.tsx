...
export const StaffDashboard = ({ filter }: StaffDashboardProps) => {
  ...
  // Escuta pedidos em tempo real no Supabase
  useEffect(() => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          cancelled_by: userData?.name || userData?.email || 'Admin',
        })
        .eq('id', cancelOrderId);
      if (error) throw error;
    ...
    <div className="dashboard-layout animate-fade-in">
      {!hasAnyFunction && userData?.role === 'staff' ? (
        <div className="alert-box" style={{ textAlign: 'center', padding: '2rem' }}>
          ...
          <p>Solicite ao Administrador para ativar suas permissões (Cozinheiro, Caixa, Atendente ou Entregador) via checkbox no banco de dados.</p>
        </div>
      ) : (
