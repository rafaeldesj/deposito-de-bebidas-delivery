...
const STORE_COORDS: [number, number] = [-22.9112951, -43.5602961];
...
const OrderMap = ({ orderId, address, deliveryCoords, clientCoords }: OrderMapProps) => {
  ...
  useEffect(() => {
    ...
    markersRef.current.origin = L.marker(STORE_COORDS, { icon: originIcon })
      .addTo(map)
      .bindPopup('<b>Depósito de Bebidas</b><br/>Suas bebidas geladas saíram daqui! 🍻');

    markersRef.current.destination = L.marker(destCoords, { icon: destIcon })
      .addTo(map)
      .bindPopup(`<b>Sua Casa</b><br/>Endereço de Entrega`);

    const currentLoc: [number, number] = deliveryCoords ? [deliveryCoords.lat, deliveryCoords.lng] : STORE_COORDS;
    ...
    const coordsStr = `${STORE_COORDS[1]},${STORE_COORDS[0]};${currentLoc[1]},${currentLoc[0]};${destCoords[1]},${destCoords[0]}`;
    const url = `https://router.project-osrm.org/route/v1/driving/${coordsStr}?overview=full&geometries=geojson`;

    fetch(url)
      .then(res => res.json())
      .then(data => {
        let routePoints: [number, number][] = [STORE_COORDS, currentLoc, destCoords];
      ...
      .catch(err => {
        ...
        const points = [STORE_COORDS, currentLoc, destCoords];
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          change_for: paymentMethod === 'dinheiro' && changeFor ? parseFloat(changeFor.replace(',', '.')) : null,
        })
        .eq('id', order.id!);
      if (error) throw error;
...
export const OrderTracking = () => {
    ...
    const loadConfig = async () => {
      try {
        const { data } = await supabase
          .from('settings')
          .select('value')
          .eq('key', 'tables_config')
          .maybeSingle();
        if (data && data.value && typeof (data.value as any).activeTablesCount === 'number') {
          setActiveTablesCount((data.value as any).activeTablesCount);
        }
      } catch (err) {
        console.error("Erro ao carregar tables_config:", err);
      }
    };
    loadConfig();

    const channel = supabase
      .channel('tables_config_tracking')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'settings', filter: 'key=eq.tables_config' }, loadConfig)
      .subscribe();
  ...
  const handleLinkOrderTable = async (tableNumStr: string) => {
    ...
    try {
      ...
      sessionStorage.setItem('deposito_mesa', tableNumStr);
  ...
  const startSimulation = async (order: OrderDocument) => {
    ...
    let dest: [number, number] = STORE_COORDS;
    ...
    try {
      const coordsStr = `${STORE_COORDS[1]},${STORE_COORDS[0]};${dest[1]},${dest[0]}`;
      const url = `https://router.project-osrm.org/route/v1/driving/${coordsStr}?overview=full&geometries=geojson`;
    ...
    await supabase.from('orders').update({
      status: 'delivering',
      delivery_coords: { lat: STORE_COORDS[0], lng: STORE_COORDS[1] }
    }).eq('id', order.id);
    ...
    const interval = setInterval(async () => {
      ...
      let currentLat = STORE_COORDS[0];
      let currentLng = STORE_COORDS[1];
      ...
      if (routePoints.length > 0) {
        ...
        currentLat = STORE_COORDS[0] + (dest[0] - STORE_COORDS[0]) * ratio;
        currentLng = STORE_COORDS[1] + (dest[1] - STORE_COORDS[1]) * ratio;
      }

      await supabase.from('orders').update({
        delivery_coords: { lat: currentLat, lng: currentLng }
      }).eq('id', order.id);
    }, 1000);
  };
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        if (error) throw error;
        if (data) {
          const mapped: OrderDocument[] = data.map((o: any) => ({
            id: o.id,
            clientUid: o.client_uid,
            ...
            address: o.address,
            deliveryUid: o.delivery_uid,
            deliveryName: o.delivery_name,
            deliveryCoords: o.delivery_coords,
            clientCoords: o.client_coords,
            dailySeq: o.daily_seq,
    ...
    // Assina realtime no Supabase
    const channel = supabase
      .channel('public:orders_tracking_client')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'orders', filter: `client_uid=eq.${user.uid}` }, fetchOrders)
      .subscribe();
                  ...
                  <div style={{ background: 'rgba(0,0,0,0.15)', padding: '0.85rem 1rem', borderRadius: '10px', marginTop: '1.5rem', fontSize: '0.9rem' }}>
                    {isDelivery ? (
                      ...
                      <div>
                        ...
                        <p style={{ margin: '0.25rem 0 0 0', color: 'var(--text-secondary)' }}>
                          O seu pedido será servido nas mesas do Depósito de Bebidas {order.tableNumber ? `(Mesa ${order.tableNumber})` : ''} (Rua Jícara, 239 - Campo Grande). Pode vir vindo!
                        </p>
                      ...
                      <div>
                        ...
                        <p style={{ margin: '0.25rem 0 0 0', color: 'var(--text-secondary)' }}>
                          Retire seu pedido diretamente no Depósito de Bebidas (Rua Jícara, 239 - Campo Grande).
                        </p>
                      </div>
                    )}
                  </div>
