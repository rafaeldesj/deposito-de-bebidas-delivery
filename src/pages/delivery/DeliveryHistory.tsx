...
export const DeliveryHistory = () => {
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      try {
        const { data, error } = await supabase
          ...
          .select('*')
          .eq('status', 'completed')
          .eq('delivery_uid', user.uid);
        if (error) throw error;

        const fetched: OrderDocument[] = (data || []).map(mapOrderFromDb);
    ...
    // Subscribe to realtime updates on public:orders for completed orders assigned to this delivery user
    const channel = supabase
      .channel(`delivery_history:${user.uid}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'orders', filter: `delivery_uid=eq.${user.uid}` }, loadHistory)
      .subscribe();
