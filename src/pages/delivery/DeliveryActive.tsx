...
const MiniMap = ({ orderId, address, clientCoords }: MiniMapProps) => {
  ...
  useEffect(() => {
    ...
    geocodeAddress(address.street, address.number, address.neighborhood)
      .then(setDestCoords)
      .catch(() => setDestCoords(STORE_COORDS));
  ...
  useEffect(() => {
    ...
    markersRef.current.origin = L.marker(STORE_COORDS, { icon: mkOrigin })
      .addTo(map).bindPopup('<b>Depósito de Bebidas</b>');
    markersRef.current.dest = L.marker(destCoords, { icon: mkDest })
      .addTo(map).bindPopup('<b>Destino de Entrega</b>');

    // Rota via OSRM
    const coordsStr = `${STORE_COORDS[1]},${STORE_COORDS[0]};${destCoords[1]},${destCoords[0]}`;
    fetch(`https://router.project-osrm.org/route/v1/driving/${coordsStr}?overview=full&geometries=geojson`)
      .then(r => r.json())
      .then(data => {
        let pts: [number, number][] = [STORE_COORDS, destCoords];
      ...
      .catch(() => {
        const pts: [number, number][] = [STORE_COORDS, destCoords];
        if (markersRef.current.poly) map.removeLayer(markersRef.current.poly);
        markersRef.current.poly = L.polyline(pts, { color: '#e28743', weight: 4, opacity: 0.85 }).addTo(map);
        map.fitBounds(L.latLngBounds(pts), { padding: [20, 20] });
      });
    ...
    map.on('click', () => {
      window.open(
        `https://www.google.com/maps/dir/?api=1&origin=${STORE_COORDS[0]},${STORE_COORDS[1]}&destination=${destCoords[0]},${destCoords[1]}&travelmode=driving`,
        '_blank'
      );
    });
  }, [destCoords]);
...
const ActiveMap = ({ address, clientCoords, gpsCoords }: ActiveMapProps) => {
  ...
  useEffect(() => {
    ...
    const map = mapRef.current;
    const riderLoc: [number, number] = gpsCoords ?? STORE_COORDS;
    ...
    markersRef.current.origin = L.marker(STORE_COORDS, { icon: mkOrigin })
      .addTo(map).bindPopup('<b>Depósito de Bebidas</b><br/>Ponto de Partida');
    ...
    const coordsStr = `${STORE_COORDS[1]},${STORE_COORDS[0]};${riderLoc[1]},${riderLoc[0]};${destCoords[1]},${destCoords[0]}`;
    fetch(`https://router.project-osrm.org/route/v1/driving/${coordsStr}?overview=full&geometries=geojson`)
      .then(r => r.json())
      .then(data => {
        let pts: [number, number][] = [STORE_COORDS, riderLoc, destCoords];
        if (data.routes?.length) {
          pts = data.routes[0].geometry.coordinates.map((c: any) => [c[1], c[0]]);
        }
        if (markersRef.current.poly) map.removeLayer(markersRef.current.poly);
      ...
      .catch(() => {
        const pts: [number, number][] = [STORE_COORDS, riderLoc, destCoords];
        if (markersRef.current.poly) map.removeLayer(markersRef.current.poly);
        markersRef.current.poly = L.polyline(pts, {
          color: '#f59e0b', weight: 5, opacity: 0.85, dashArray: '8, 12',
        }).addTo(map);
...
export const DeliveryActive = () => {
  ...
  // Escuta todos os pedidos em tempo real do Supabase
  useEffect(() => {
    ...
    const channel = supabase
      .channel('public:orders_delivery_active')
          ...
          const saveGps = async () => {
            try {
              await supabase.from('orders').update({
                delivery_coords: { lat: latitude, lng: longitude },
  ...
  const handleAcceptDelivery = async (orderId: string) => {
    try {
      await supabase.from('orders').update({
        status: 'delivering',
        delivery_uid: user?.uid,
        delivery_name: user?.displayName || user?.email || 'Entregador',
