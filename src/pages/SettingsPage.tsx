...
export const SettingsPage = () => {
  ...
  // Load audit logs from Supabase audit_logs table
  useEffect(() => {
    ...
    const loadAuditLogs = async () => {
      try {
        ...
        // Map snake_case database schema to camelCase front-end model if needed
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'audit_logs' }, loadAuditLogs)
      .subscribe();
  ...
  // Load orders for commissions calculation from Supabase
  useEffect(() => {
    if (activeTab !== 'commissions' || !isAdmin) return;
    
    setLoadingOrders(true);
    
    const loadOrdersForCommissions = async () => {
      try {
        const { data, error } = await supabase
          .from('orders')
          .select('*')
    ...
    loadOrdersForCommissions();

    const channel = supabase
      .channel('public:orders_commissions')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, loadOrdersForCommissions)
      .subscribe();
  ...
  const handleSaveDevMPConfig = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fetch existing config and merge
      ...
      const { error } = await supabase
        .from('settings')
        .upsert({ key: 'store_config', value: mergedConfig });
      if (error) throw error;
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Update address & CPF in Supabase user document
      const { error } = await supabase
        .from('users')
        .update(mapUserToDb({
  ...
  const handleToggleStoreOpen = async () => {
    ...
    try {
      // 2. Grava no Supabase imediatamente
      const { data: current } = await supabase
        .from('settings')
        .select('value')
        .eq('key', 'store_config')
  ...
  const handleSaveStoreConfig = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase
        .from('settings')
        .upsert({ key: 'store_config', value: storeConfig });
      if (error) throw error;
  ...
  return (
    <div className="dashboard-layout animate-fade-in" style={{ paddingBottom: '3rem' }}>
      <div className="dashboard-header">
        ...
        <p>Gerencie seus dados de perfil, endereços de entrega e configurações operacionais do depósito de bebidas.</p>
      </div>
