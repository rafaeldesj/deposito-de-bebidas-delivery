...
const mapCameraFromDb = (row: any): SecurityCamera => {
  return {
    id: row.id,
    name: row.name,
    brand: row.brand,
    ...
    useCloudStream: row.use_cloud_stream,
    cloudProvider: row.cloud_provider,
    cloudStreamUrl: row.cloud_stream_url,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
};

const mapCameraToDb = (cam: Partial<SecurityCamera>) => {
  const dbData: any = {};
  if (cam.id !== undefined) dbData.id = cam.id;
  if (cam.name !== undefined) dbData.name = cam.name;
  if (cam.brand !== undefined) dbData.brand = cam.brand;
  if (cam.ip !== undefined) dbData.ip = cam.ip;
  if (cam.port !== undefined) dbData.port = cam.port;
  if (cam.username !== undefined) dbData.username = cam.username;
  if (cam.password !== undefined) dbData.password = cam.password;
  if (cam.streamType !== undefined) dbData.stream_type = cam.streamType;
  if (cam.streamPath !== undefined) dbData.stream_path = cam.streamPath;
  if (cam.wifiSSID !== undefined) dbData.wifi_ssid = cam.wifiSSID;
  if (cam.macAddress !== undefined) dbData.mac_address = cam.macAddress;
  if (cam.deviceId !== undefined) dbData.device_id = cam.deviceId;
  if (cam.useCloudStream !== undefined) dbData.use_cloud_stream = cam.useCloudStream;
  if (cam.cloudProvider !== undefined) dbData.cloud_provider = cam.cloudProvider;
  if (cam.cloudStreamUrl !== undefined) dbData.cloud_stream_url = cam.cloudStreamUrl;
  if (cam.createdAt !== undefined) dbData.created_at = cam.createdAt;
  if (cam.updatedAt !== undefined) dbData.updated_at = cam.updatedAt;
  return dbData;
};
import { 
...
export const SecurityCameraSettings = () => {
  ...
  useEffect(() => {
    ...
    const loadCameras = async () => {
      try {
        ...
        const mapped = (data || []).map(mapCameraFromDb);
        setCameras(mapped);

        // If no cameras exist, seed the default Yoosee camera automatically
        if (mapped.length === 0 && isDev) {
          const defaultCamera: SecurityCamera = {
            id: 'yoosee_sala',
            name: 'Sala',
            brand: 'Yoosee',
            ip: '192.168.1.2',
          ...
          await supabase.from('cameras').insert(mapCameraToDb(defaultCamera));
    ...
    // Subscribe to realtime updates on public:cameras
    const channel = supabase
      .channel('public:cameras_updates')
  ...
  const handleSaveCamera = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isAdding) {
        ...
        const finalCamera = { id: newId, ...cameraData };
        const { error } = await supabase.from('cameras').insert(mapCameraToDb(finalCamera));
        if (error) throw error;
  ...
  const handleDeleteCamera = async (camId: string, camName: string) => {
    ...
    try {
      const { error } = await supabase.from('cameras').delete().eq('id', camId);
      if (error) throw error;
