...
function nativeRequest(url: string, method: string, headers: any, data?: any): Promise<any> {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // 1. Obter preços reais dos produtos direto do Supabase para evitar fraude
    ...
    // Recalcular subtotal com base nos preços do banco
    ...
    let gatewayResponse: any = { success: true };
    ...
    if (paymentMethod === 'pix') {
      ...
      if (isMock) {
        ...
        // Chamada real PagBank
        const headers = {
          'Authorization': `Bearer ${pagbankToken}`,
          'Content-Type': 'application/json'
        };
        const baseUrl = 'https://sandbox.api.pagseguro.com'; // Ou produção se configurado
        ...
        // Salvar cartão no cofre se consentido
        if (!useSavedCard && saveCardConsent) {
    ...
    // 6. Criar registro de pedido no Supabase
    // Calcular sequência diária
    ...
    if (insertErr) {
      console.error('Erro ao gravar pedido no Supabase:', insertErr);
      return res.status(500).json({ success: false, message: 'Pedido pago/gerado, mas erro ao gravar no banco.' });
    }
    ...
    console.error('[Checkout Serverless] Erro no processamento:', error);
    return res.status(500).json({ success: false, message: error.message || 'Erro interno no servidor ao processar o checkout.' });
  }
}
